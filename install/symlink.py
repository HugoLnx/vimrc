...
def lua_string_list(items):
    quoted = ', '.join("'" + item.replace('\\', '\\\\').replace("'", "\\'") + "'" for item in items)
    return '{ ' + quoted + ' }'


def local_nvim_config_content(csharp_lsp, allowed_dirs):
    value = 'true' if csharp_lsp else 'false'
    windows_dirs = lua_string_list(allowed_dirs.get('windows') or [])
    linux_dirs = lua_string_list(allowed_dirs.get('linux') or [])
    return (
        "-- Generated by install/symlink.py from config.yml's `csharp_lsp` and\n"
        "-- `allowed_dirs` options. Do not edit directly; edit config.yml and\n"
        "-- re-run the script.\n"
        "return {\n"
        f"  csharp_lsp = {value},\n"
        "  allowed_dirs = {\n"
        f"    windows = {windows_dirs},\n"
        f"    linux = {linux_dirs},\n"
        "  },\n"
        "}\n"
    )
...
def sync_local_nvim_config(csharp_lsp, allowed_dirs, dry_run):
    target = REPO_ROOT / 'nvim' / 'lua' / 'user' / 'local.lua'
    content = local_nvim_config_content(csharp_lsp, allowed_dirs)
    if target.is_file() and target.read_text() == content:
        print(f"  up to date: {target}")
        return
...
def main():
    ...
    sync_local_nvim_config(data.get('csharp_lsp', True), data.get('allowed_dirs') or {}, args.dry_run)
